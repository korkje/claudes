...
For a pre-release, tag with a suffix (`v0.2.0-next.0`) and tick "Set as a pre-release": anything marked pre-release publishes under the `next` dist-tag without moving `latest`, so `npm i -g @korkje/claudes@next` gets it.
